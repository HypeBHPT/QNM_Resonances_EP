		...
		for(iDom=0; iDom<nDoms; iDom++){
			...
			for(i1=0; i1<=N1; i1++){
				...
				if(i1==N1 && iDom!=0){ // Transition_Condition_Left: Continuity on Function
					derivs W_grid_prevDom, W_grid_sigma, W_grid_prevDom_sigma;
					allocate_derivs(&W_grid_prevDom, 0, NFields);
					allocate_derivs(&W_grid_sigma, 0, NFields);
					allocate_derivs(&W_grid_prevDom_sigma, 0, NFields);
					...
					scale_derivs_x0x1_to_tau_sigma(par, iDom, i1, W_grid_prevDom, W_grid_prevDom_sigma, nFields);
					
					double Re_u_Dom  = W_grid_sigma.X[0], Re_u_dt_Dom  = W_grid_sigma.Y[0], 
							   Im_u_Dom  = W_grid_sigma.X[1], Im_u_dt_Dom  = W_grid_sigma.Y[1], 
							   Re_du_Dom = W_grid_sigma.X[2],  
							   Im_du_Dom = W_grid_sigma.X[3], 
							   Re_u_PrevDom  = W_grid_prevDom_sigma.X[0], 
							   Im_u_PrevDom  = W_grid_prevDom_sigma.X[1];

							   F_2D_Equations[0] = Re_du_Dom - Re_u_dt_Dom;
							   F_2D_Equations[1] = Im_du_Dom - Im_u_dt_Dom;
							   F_2D_Equations[2] = Re_u_Dom - Re_u_PrevDom - jump_field(par, 0, x0);
							   F_2D_Equations[3] = Im_u_Dom - Im_u_PrevDom - jump_field(par, 1, x0);
					...
					free_derivs(&W_grid_prevDom_sigma, 0, NFields);
				}
				else if(i1==0 && iDom!=nDoms-1){// Transition_Condition_Right: Continuity on Function's derivative
					derivs W_grid_NextDom, W_grid_Dom_sigma, W_grid_NextDom_sigma;
		
					allocate_derivs(&W_grid_Dom_sigma, 0, NFields);
					scale_derivs_x0x1_to_tau_sigma(par, iDom, i1, W_grid, W_grid_Dom_sigma, nFields);
					...
					double Re_u_sigma_Dom  = W_grid_Dom_sigma.X1[0], Re_u_sigma_NextDom  = W_grid_NextDom_sigma.X1[0], 
							   Im_u_sigma_Dom  = W_grid_Dom_sigma.X1[1], Im_u_sigma_NextDom  = W_grid_NextDom_sigma.X1[1],
							   Re_u_dt_Dom  = W_grid_NextDom_sigma.Y[0], Re_du_Dom = W_grid_NextDom_sigma.X[2],  
							   Im_u_dt_Dom  = W_grid_NextDom_sigma.Y[1], Im_du_Dom = W_grid_NextDom_sigma.X[3];			   

							   F_2D_Equations[0] = Re_du_Dom - Re_u_dt_Dom;
							   F_2D_Equations[1] = Im_du_Dom - Im_u_dt_Dom;
							   F_2D_Equations[2] = Re_u_sigma_NextDom-Re_u_sigma_Dom - jump_derivative(par, 0, x0);
							   F_2D_Equations[3] = Im_u_sigma_NextDom - Im_u_sigma_Dom - jump_derivative(par, 1, x0);
					...
					free_derivs(&W_grid_Dom_sigma, 0, NFields);
				...
				// printf("tau=%3.15e\t sigma=%3.15e\t phi=%3.15e\t F[0]=%3.15e\t F[2]=%3.15e \n", tau, sigma, W_grid.X[0], F_2D_Equations[0], F_2D_Equations[2]);
				// printf("tau=%3.15e\t sigma=%3.15e\t phi,sigma=%3.15e\t F[0]=%3.15e\t F[1]=%3.15e F[2]=%3.15e\t F[3]=%3.15e \n", tau, sigma, W_grid_sigma.X1[0], F_2D_Equations[0], F_2D_Equations[1], F_2D_Equations[2], F_2D_Equations[3]);
				// printf("tau=%3.15e\t sigma=%3.15e\t phi=%3.15e\t phi,sig=%3.15e\t phi,sig=%3.15e \n", tau, sigma, W_grid_sigma.X[0], W_grid_sigma.X1[0], W_grid_sigma.X11[0]);
				// printf("tau=%3.15e\t sigma=%3.15e\t psi=%3.15e\t psi,sig=%3.15e\t psi,sig=%3.15e \n", tau, sigma, W_grid_sigma.X[2], W_grid_sigma.X1[2], W_grid_sigma.X11[2]);
			}
			// pause();
	...
	for(i0=0; i0<=N0; i0++){
		...
		for(iDom=0; iDom<nDoms; iDom++){
			...
			for(i1=0; i1<=N1; i1++){
				...
				if(i1==N1 && iDom!=0){ // Transition_Condition_Left: Continuity on Function
					derivs W_grid_prevDom,  W_grid_sigma,  W_grid_prevDom_sigma,
							  DW_grid_prevDom, DW_grid_sigma, DW_grid_prevDom_sigma;
					...
					allocate_derivs(&W_grid_prevDom_sigma, 0, NFields); allocate_derivs(&DW_grid_prevDom_sigma, 0, NFields);

					Get_Fields2D_atGrid( par, iDom-1, i0, 0, ID,  V, W_grid_prevDom);
					Get_Fields2D_atGrid( par, iDom-1, i0, 0, Null_ID,  V, DW_grid_prevDom);
					...
					scale_derivs_x0x1_to_tau_sigma(par, iDom, i1, DW_grid_prevDom, DW_grid_prevDom_sigma, nFields);


					double Re_Du_Dom  = DW_grid_sigma.X[0], Re_Du_dt_Dom  = DW_grid_sigma.Y[0], 
							   Im_Du_Dom  = DW_grid_sigma.X[1], Im_Du_dt_Dom  = DW_grid_sigma.Y[1], 
							   Re_Ddu_Dom = DW_grid_sigma.X[2],  
							   Im_Ddu_Dom = DW_grid_sigma.X[3], 
							   Re_Du_PrevDom  = DW_grid_prevDom_sigma.X[0], 
							   Im_Du_PrevDom  = DW_grid_prevDom_sigma.X[1];

							   JDX_2D_Equations[0] = Re_Ddu_Dom - Re_Du_dt_Dom;
							   JDX_2D_Equations[1] = Im_Ddu_Dom - Im_Du_dt_Dom;
							   JDX_2D_Equations[2] = Re_Du_Dom  - Re_Du_PrevDom;
							   JDX_2D_Equations[3] = Im_Du_Dom  - Im_Du_PrevDom;
					...
					free_derivs(&W_grid_prevDom_sigma, 0, NFields); free_derivs(&DW_grid_prevDom_sigma, 0, NFields);
				}
				else if(i1==0 && iDom!=nDoms-1){// Transition_Condition_Right: Continuity on Function's derivative
					derivs W_grid_NextDom, DW_grid_NextDom, 
								 W_grid_Dom_sigma, DW_grid_Dom_sigma, 
								 W_grid_NextDom_sigma, DW_grid_NextDom_sigma;
		
					allocate_derivs(&W_grid_Dom_sigma, 0, NFields); allocate_derivs(&DW_grid_Dom_sigma, 0, NFields);
					scale_derivs_x0x1_to_tau_sigma(par, iDom, i1, W_grid, W_grid_Dom_sigma, nFields);
					scale_derivs_x0x1_to_tau_sigma(par, iDom, i1, DW_grid, DW_grid_Dom_sigma, nFields);
					...
					double Re_Du_sigma_Dom  = DW_grid_Dom_sigma.X1[0], Re_Du_sigma_NextDom  = DW_grid_NextDom_sigma.X1[0], 
							   Im_Du_sigma_Dom  = DW_grid_Dom_sigma.X1[1], Im_Du_sigma_NextDom  = DW_grid_NextDom_sigma.X1[1],
							   Re_Du_dt_Dom  = DW_grid_NextDom_sigma.Y[0], Re_Ddu_Dom = DW_grid_NextDom_sigma.X[2],  
							   Im_Du_dt_Dom  = DW_grid_NextDom_sigma.Y[1], Im_Ddu_Dom = DW_grid_NextDom_sigma.X[3];			   

					JDX_2D_Equations[0] = Re_Ddu_Dom - Re_Du_dt_Dom;
					JDX_2D_Equations[1] = Im_Ddu_Dom - Im_Du_dt_Dom;
					JDX_2D_Equations[2] = Re_Du_sigma_NextDom-Re_Du_sigma_Dom;
					JDX_2D_Equations[3] = Im_Du_sigma_NextDom - Im_Du_sigma_Dom;
					...
					free_derivs(&W_grid_Dom_sigma, 0, NFields); free_derivs(&DW_grid_Dom_sigma, 0, NFields);
